...
import { loadInstances } from './load-instances.js'
import {
...
export class FileInstanceStore implements InstanceStore {
	...
	private readonly monsterContent
	private readonly characterClassContent
	...
	constructor() {
		...
		this.itemContent = content.itemContent
		this.monsterContent = content.monsterContent
		this.characterClassContent = content.characterClassContent
	...
	private hydrateItemInstance(instance: ItemInstance): EquipmentItem {
		const blueprint = this.itemContent.get(instance.blueprintId)
	...
	private hydrateMonsterInstance(instance: MonsterInstance): Monster {
		const blueprint = this.monsterContent.get(instance.blueprintId)
	...
	private hydrateCharacterClassInstance(
		...
		const blueprint = this.characterClassContent.get(instance.blueprintId)
	...
	private requireItemBlueprint(blueprintId: string) {
		const blueprint = this.itemContent.get(blueprintId)
	...
	private requireMonsterBlueprint(blueprintId: string) {
		const blueprint = this.monsterContent.get(blueprintId)
	...
	private requireCharacterClassBlueprint(blueprintId: string) {
		const blueprint = this.characterClassContent.get(blueprintId)
